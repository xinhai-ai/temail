...
export async function PATCH(
  ...
  const authResult = await authenticateOpenApiRequest(request, { requiredScopes: "tags:write" });
  if (!authResult.ok) {
    return NextResponse.json({ error: authResult.error }, { status: authResult.status });
  }
  ...
  try {
    ...
    const addNames = Array.from(new Set((data.add || []).map((t) => t.trim()).filter(Boolean)));
    const removeIds = Array.from(new Set((data.remove || []).map((t) => t.trim()).filter(Boolean)));
    ...
    const tags = await prisma.$transaction(async (tx) => {
      if (addNames.length > 0) {
        const ensured = await Promise.all(
          addNames.map((name) =>
            tx.tag.upsert({
              where: { userId_name: { userId: authResult.apiKey.userId, name } },
              create: { userId: authResult.apiKey.userId, name },
              update: {},
              select: { id: true },
            })
          )
    ...
    console.error("[open/v1/emails/tags] update failed:", error);
